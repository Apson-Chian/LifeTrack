...
- 自定义地点、分类、半径、收藏和校园地点；校园、图书馆、教室、寝室、美食街支持手绘不规则边界
- 实时地点识别与基于完整轨迹的停留重建
- PhotoKit 元数据读取与本地 Vision 照片分类缓存
- 照片 Vision 分类默认使用低负载后台队列，可切换均衡速度；云端 AI 只处理文字和用户授权的位置数据
...
- 不上传照片，不使用服务器账号系统；启用 AI 且用户单独允许“照片地点”后，可发送受限工具产生的照片地点名称、精确坐标、约距离、文字/数值和安全摘要。
...
- “助手”标签页支持连续对话，按轮次独立成卡且最新回答置顶；也可一键生成今日回顾、本周复盘、学娱平衡和旅行手记。
...
- **照片地点按需授权**：默认不向 AI 提供照片地点。开启后，地点关键词会先由 Apple 地图解析，再由本机按照片 GPS 筛选附近记录；模型可收到地点名称、精确坐标、约距离、拍摄时间和安全文字摘要。
- **隐私红线**：原图和缩略图永不交给 AI；人物、自拍、资产标识符和文件路径始终被过滤。两个渠道共用同一个纯文本请求结构。
