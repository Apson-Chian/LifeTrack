...
- “助手”标签页支持针对整个 App 记录的自由问答，也可一键生成今日回顾、本周复盘、学娱平衡和旅行手记。
- 今日运动、轨迹、地点、课表、照片智能整理和旅行归档页面都有带业务上下文的助手入口。
- 实现为一个**本地工具调用 Agent**：模型通过受限工具按需读取活动、轨迹、出行、停留、地点名称、课表、学习停留、旅行归档和脱敏照片摘要，再生成自然语言回答。
- **隐私红线**：原图和缩略图永不交给 AI。`PhotoAnalysisRecord` 只允许经过本机聚合脱敏后输出非人物类别与通用标签统计；人物、自拍、标识符、路径、坐标、拍摄时间和单张照片详情均被过滤。Agnes 客户端消息结构只携带 `String` 文本。
